...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(time, week) in bookstores.openingHours" :key="week">
          {{ week }} (open: {{ time.open }}, close: {{ time.close }})
        </li>
      </ul>
    ...
    <section class="lab-section">
      ...
      <p :class = "{highlight: isHighlight('George Orwell')}">George Orwell</p>
      <p :class = "{highlight: isHighlight('Jane Austen')}">Jane Austen</p>
      </p>
    </section>
  </div>
</template>
...
// check the content
const isHighlight = (text) => text === "George Orwell"
</script>
